...
class WeightIndividual(object):
    ...
    def clone(self):
        new_obj = WeightIndividual(self.NC,self.ND,self.force)
        new_obj.fitness = deepcopy(self.fitness)  
...
def mutate_weights(ind,force):
    ...
    new_solution[sel_cluster,sel_dim] += np.random.normal(loc=0.0, scale=0.1)
    new_solution[sel_cluster,sel_dim] = np.clip(new_solution[sel_cluster,sel_dim],0.0000001,1.0)
    new_solution[sel_cluster,:] =  fix_weights(new_solution[sel_cluster,:],force)
...
def crossover_weights(ind1,ind2,force):
    ...
    child1, child2 = tools.cxUniform(ind1.weights.flatten(),ind2.weights.flatten(),indpb=0.5)
